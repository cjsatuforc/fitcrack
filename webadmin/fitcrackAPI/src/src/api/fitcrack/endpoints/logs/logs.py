...
class logs(Resource):
    ...
    def get(self):
        ...
        while True:
            logs = shellExec('head -n -' + str(skip) + ' /app/log_teserakt/teserakt_work_generator.log | tail -n 100')
            if logs == '':
                return {'items': []}
            logs = logs.split('\n')
            results = []
...
class logs(Resource):
    ...
    def get(self):
        ...
        skip = 0
        while True:
            fewLastLogs = shellExec('head -n -' + str(skip) + ' /app/log_teserakt/teserakt_work_generator.log | tail -n 100')
            if fewLastLogs == '':
                return {'items': []}
...
#         args = newLogs_argument.parse_args(request)
#         lastLog = args.get('log', 1)
#         fewLastLogs = shellExec('tail -n 100 /app/log_teserakt/teserakt_work_generator.log')
#         fewLastLogs = fewLastLogs.split('\n')
#         newLogsCount = 0
#         for log in reversed(fewLastLogs):
#             if (lastLog == log):
